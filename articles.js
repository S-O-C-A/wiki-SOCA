/* ==========================================================================
   КОДЕКС ПАНДЕМОНИУМ // PANDEMONIUM OS — реестр статей
   Данные пилотов (Коко/Клаудия/Альфа) извлечены из pilots_data.js.
   Марай — в отдельном лоре, досье в этот файл не входило.
   Правь текст в объектах; ссылки <a href="#/id"> открывают окна.
   gallery: N — число фото в карусели инфобокса (файлы картинки/<id>-N.png).
   ========================================================================== */

/* Журнал обновлений для заглавной. Заполни: {date:'2026-09-03', text:'что изменилось'} */
window.WIKI_UPDATES = [
  // { date: '2026-09-03', text: 'Пример записи — удали и впиши свои.' },
];
...
window.WIKI_ARTICLES = [

  {
    ...
    emblem: 'i',
    gallery: 2,
    intro: `<p class="lead"><strong>КОДЕКС СОКА</strong> - авторская энциклопедия проекта <a href="#/pandemonium-04">СОКА</a> и её сайта <a href="#/soca-site">socasystem.space</a>. Раздел "База" — о самой вики и о сайте вне лора: кто автор, как всё делалось, что уже вышло и что впереди.</p>`,
    sections: [
      { h: 'О вики',
        html: `<p>Официальная википедия от создателя сайта СОКА. Вики ведётся вручную и обновляется по мере развития сайта и его обновлений.</p>
      <div class="note"><b>Отдельное упоминание.</b> Стоит сразу отметить, что помимо Яна также участвовали и другие люди в развитии сайта. Передаю привет Жене и Шарлотте, спасибо, что вложили вклад в сайт и предоставили своих персонажей! (сайт не присваивает себе авторство чужих персонажей, и полную ответственность за них несут их авторы).</div>` },
      { h: 'О сайте вне лора',
        html: `<p>Основной сайт — это интерактивный опыт в стиле CRT-терминала: ретро-эстетика "утраченного носителя", где с тобой говорит бортовой ИИ. Сайт имеет несколько уровней кроме основного: главное меню, досье пилотов, сектор 7. На данный момент только 3.</p>
      <div class="note"><b>Производство.</b> Сайт начал свою первую разработку в начале мая 2026 года как подготовка к будущему учебному проекту. Впервые сайт был выставлен 15 сентября этого же года. Разработка заняла 4 месяца и до сих пор находится в разработке.</div>` },
      { h: 'Что происходит на сайте',
        html: `<p>Пользователи подключаются как пилот к кораблю, и их встречают ИИ. У сайта нет никаких целей на первый взгляд — просто сайт, созданный как имитация старых терминалов и ПО для космических кораблей. Пользователи могут исследовать сайт, взаимодействовать с системой и ИИ. Однако если копать, можно найти спрятанные тайны и пасхалки, а также углубиться в лор персонажей. Сайт в открытую ничего полностью не рассказывает, а лишь кидает намёки и подсказки. Обычным пользователям, которые изучают сайт поверхностно, всё будет казаться скучным и простым. Подробно — в разделе <a href="#/soca-site">Сайт СОКА</a>.</p>` },
      { h: 'Обновления',
        html: `<p>После первой эксплуатации сайта этот пункт будет пополняться по мере новых обновлений.</p>` },
      { h: 'Далее планируется:',
        html: `<p>Полный перевод всего сайта на русский; открытие вкладки Storage; добавление новой части сайта — отчёты; сайт-Q&amp;A; разные аккаунты пилотов.</p>` },
    ],
    seeAlso: ['soca-site'],
  },

  {
    ...
    emblem: 'С',
    gallery: 3,
    intro: `<p><strong>Сайт СОКА</strong> - основной сайт проекта: внутримировой терминал корабля <a href="#/pandemonium-04">ПАНДЕМОНИУМ-04</a>, где с тобой общается ИИ <a href="#/soca">СОКА</a>. Отсюда открываются подсайты: <a href="#/sub-dossier">Досье пилотов</a>, <a href="#/sub-sector7">Сектор 7</a> и <a href="#/sub-reports">Репортс</a>. Основные цвета сайта темно-зеленый и голубой, а узнаваемым символом является шестеренка и гоночный флаг, также присутствуют эффекты шума как в телевизорах, сканеры, и намеренные баги и глитчи, тем самым показывая, что СОКА сама по себе не исправна. На сайте присутствуют звуки, и в основном мы слышим музыку с гоночного автострайка, подчеркивая атмосферу сайта.</p>`,
    sections: [
      { h: 'Что это',
        html: `<p>Диегетический интерфейс: пользователь - пилот, вошедший в систему корабля. СОКА реагирует на действия, ведёт события и подаёт информацию в своей холодной саркастичной манере. Эстетика — "лост-медиа", испорченный носитель.</p>` },
      { h: 'Бут-экран',
        html: `<p>Вход через загрузочный экран: вводятся <em>название корабля</em>, <em>имя пилота</em> и <em>пароль</em>. После ввода пользователь попадает на аккаунт <a href="#/koko">Кокоро</a>. При ошибке ввода пароля, СОКА выдаст только саркастичный комментарий, а при вводе имени Альфа или Клавдия, СОКА отклонит эти имена с комментарием "ты не Альфа/Клавдия", а в строку пароля можно ввести все что угодно и мы попадем уже на сам сайт. Таким образом подразумевается, что вводя эти данные, мы обозначаем конкретный корабль в всемирной сети СОКА, определенного пилота, зарегистрированного в выбранном корабле, и вводим пароль, какой поставил пилот на свой аккаунт. И судя по всему, Коко явно не переживает, что любой человек может зайти на его аккаунт.</p>
      ` },
      { h: 'Разделы, Ф-ки, чат, игры, тосты, попапы',
        html: `<p>На сайте есть заголовки-разделы, набор функций ("Ф-ки"), чат с ИИ, мини-игры, всплывающие уведомления (тосты), попапы и многое другое.</p>
        <p>На сайте присутствуют такие окна как: // TELEMETRY, // PROXIMITY SCAN, // COMMS, // PILOT DATA и // SYS STATUS, которые сопровождаются во всех Ф-ках, а в самой // NAVIGATION (Ф1) находятся все остльные 9 Ф-ок, а также присутстует шапка, в которой транслируется реальная дата (с годом 1974) и время пользователя, и подвал, в котором можно увидеть закрытый S7.SECTOR.</p>
        <p>DASHDOARD - панель управления, оно же и обозначается как Ф1. Главная страница, которая встречает пользователей. Там размещены быстрые данные о корабле и его состоянии, такие как: статус ядер, контроль миссии, векторное движение и предупреждения.</p>
        <p>ABOUT SOCA - про систему СОКА и про навигацию сайта, Ф2. Здесь рассказывается про судно, его краткое описание, про СОКА и СМАЙЛИ, упоминает экипаж, а также предоставляется объяснение всего сайта и навигация. </p>
        <p>SYS LOG - системные логи, ф3. Там транслируются системные логи самого корабля, логи СОКА и СМАЙЛИ, а также другие сообщения, которые могут не касаться логов.</p>
        <p>DIAGNOSTICS - диагностика корабля, ф4. Позволяет провести быструю диагностику состояние корабля и подвести итог. Всегда показывает 2 критических вопроса, 4 предупреждения и 6 оптимальных систем и рекомендует необходимое и немедленное обслуживание. Кто бы это еще послушал?</p>
        <p> AUTOPILOT - автопилот корабля, ф5. Здесь отображется путь корабля, пролетевшие места, ближайшие места и пункт назначения. Автопилот является деградированным  задержкой в 240мс. Здесь же можно найти и другие окна, типа проекция корабля, ручного управления, трансляция биометрии пилота, перегрузка и ориентация, аварийные протоколы, среда и жизнеобеспечение.</p>
        <p>NAV CORE -навигацонное ядро, ф6. Представлена в виед 3д карты, с которой можно взаимодействовать. Выбирая объект,  </p>
        <p> </p>
        <p> </p>
        <p> </p>
        <p> </p>
...

  {
    ...
    sections: [
      { h: 'Внешность',
        html: `<p>Бело-оранжевый скафандр. Короткие пушистые серо-фиолетовые волосы с розовыми кончиками, бело-зелёные глаза, инженерные очки с тёмно-фиолетовыми линзами.</p>` },
      { h: 'Характер',
      ...
      { h: 'Биография',
        html: `<p>Неугомонный ребёнок, угнавший корабль совсем молодым; отец поддерживал его любопытство.</p>` },
      { h: 'Отношения',
...

  {
    ...
    sections: [
      { h: 'Внешность',
        html: `<p>Бледная кожа, чёрные волосы с красным отливом, чёрные глаза, очки с оранжевыми линзами. Длинное пальто с меховой опушкой; постоянно мёрзнет.</p>` },
      { h: 'Характер',
